...
let userA; // Dados da empresa A, carregados do setup
let API_URL;
...
describe('6. Módulo de Autenticação e Sessão (Stateful)', () => {
  ...
  beforeAll(async () => {
    ...
    API_URL = setupData.apiUrl; // Carrega a URL correta da API
    // A limpeza da coleção SessionToken já é feita de forma centralizada no `test-setup.js`.
  });
