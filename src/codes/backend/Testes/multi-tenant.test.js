...
describe('3. Teste de Isolamento de Dados em Larga Escala (Multi-Tenant)', () => {
    ...
    test('deve criar uma transação para cada empresa', async () => {
        ...
        for (let i = 0; i < companies.length; i++) {
            ...
            companies[i].transactions.push(response.data);
            console.log(`Transação criada para ${company.companyName}. ID: ${response.data._id}`);
        }
    }, 20000);
    ...
    test('deve garantir que cada empresa liste apenas suas próprias transações', async () => {
        ...
        for (const company of companies) {
            ...
            expect(response.data[0]._id).toBe(company.transactions[0]._id);
